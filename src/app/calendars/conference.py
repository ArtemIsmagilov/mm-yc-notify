...
class Conference:
    ...
    def __init__(self, event: Event, timezone: str):
        ...
        self.uid: str | None = _set_uid(event.get("UID"))
        self.dtstart: datetime | None = _set_dtstart(self.timezone, event.get("DTSTART"))
        self.dtend: datetime | None = _set_dtend(self.timezone, event.get("DTEND"))
        self.summary: str | None = _set_summary(event.get("SUMMARY"))
        self.created: datetime | None = _set_created(self.timezone, event.get("CREATED"))
        self.last_modified: datetime | None = _set_last_modified(self.timezone, event.get("LAST-MODIFIED"))
        self.description: str | None = _set_description(event.get("DESCRIPTION"))
        self.url_event: str | None = _set_url_event(event.get("URL"))
        self.categories: str | None = _set_categories(event.get("CATEGORIES"))
        self.x_telemost_conference: str | None = _set_x_telemost_conference(event.get("X-TELEMOST-CONFERENCE"))
        self.organizer: dict | None = _set_organizer(event.get("ORGANIZER"))
        self.attendee: list[dict] | None = _set_attendee(event.get("ATTENDEE"))
        self.location: str | None = _set_location(event.get("LOCATION"))
        self.recurrence_id: datetime | None = _set_recurrence_id(self.timezone, event.get('RECURRENCE-ID'))
        self.conf_id: str | None = _set_conf_id(self.uid, self.recurrence_id)
