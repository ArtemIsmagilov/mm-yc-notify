...
- [x]  Установка статуса "📆 In a meeting" во время конференции. Если изначальный статус длительнее или в режиме `не стирать`,
...
- [x]  Уведомление об изменении/добавлении/удалении конференции, наличие всех доступных атрибутов участников конференции
...
1. В переменной окружения следует прописать в стиле cron частоту запроса яндекс сервера<br>
  Примеры https://crontab.guru/examples.html

2. Внимание! Администратору следует тонко настроить время повторения обновления запросов на сервер. 
Если запросы слишком частые, то приложение будет излишне перегружено, если слишком редкое, то для сотрудниковс
очень частыми конференциями информация от уведомления будет не актуальна.

3. Вы получаете уведомление об удалениях/изменения/добавлениях конференций.
...
4. При тестировании следует поменять несколько параметров, а именно:
   ...
   например пинг каждые 10 секунд, в продакшене конечно этого не стоит делать. также добавить параметр слуйчаности 
   `jitter ` - исполнение задач в случайное время в заданом парадке секунд https://apscheduler.readthedocs.io/en/latest/modules/triggers/cron.html#module-apscheduler.triggers.cron .
   Это обеспечивает стабильно при сильных нагрузках
...
5. Как работает:
   ...
   добавлена не раньше 15 минут. Почему? Потому что нужно гарантированно отправлять уведомления о событиях которые не
   ...
   ваш статус длительнее статуса конференции или стоит на режиме(постоянно), то приложение вернут ваш статус.
   4. Блок `notifications` состоит из блоков 
       - уведомление ежедневные в конкретное время
       - уведомление обо всех операция связанных с вашими конференциями, синронизация через лонг пулинг, 
...
6. Предостережение
    - Если вы при создании интеграции вводите валидный логин, токен, пользуетесь приложением, а потом меняете, 
   то интеграция сразу удаляет ваши данные из приложения как невалидный пользователь, следует либо ничего не менять,
   либо в приложении обновлять данные через команды
    - Если вы выбираете некоторые календари, а потом уваляет в яндекс календаре, ваш изначальный выбор уведомлений
   с сихронизированными конференциями уваляется как не валидные данные, опять же - обновляйте ваши изменения через 
   команды приложения
    - Конференции, которые были добавлены раньше 15 минут не будут обрабатываться приложением для обновления статуса и 
   сообщения о престоящем событии, так как невозможно уведомить о предстоящей конференции за 10 минут если вы создали 
   коференцию за 1, 3, 5 и так далее до 15 минут. 5 минут добавлены как буфер.
...
- оптимизация алгоритма обновлений конференция через CalDAV сервер
...
- Проекту [mattermostautodriver](https://github.com/embl-bio-it/python-mattermost-autodriver), форк от заброшенного 
  ...
  Документация https://embl-bio-it.github.io/python-mattermost-autodriver/ .  Есть возможность автоматически обновить конечные точки
  Mattermost API в пакете драйвера https://github.com/embl-bio-it/python-mattermost-autodriver#updating-openapi-specification 
