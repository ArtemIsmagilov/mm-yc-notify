...
2. Внимание! Администратору следует тонко настроить время повторения обновления, пинга сервера. 
Если запросы слишком частко повторяются, то приложение будет излишне перегружено, если слишком редкое, то для сотрудников с очень частыми конференциями информация от уведомлений будет не актуальна.

3. Вы получаете уведомление об удалениях/изменениях/добавлениях конференций.
...
4. При тестировании следует поменять несколько параметров, а именно:
   ...
   например пинг каждые 10 секунд, в продакшене этого не стоит делать. Также добавить параметр слуйчаности 
